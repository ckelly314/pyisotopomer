...
def calcdeltabulk(isol):
    ...
    d15N = 1000 * (isol["15Rbulk"] / 0.0036765 - 1) # 15R(air-N2) = 0.0036782 [De Bièvre et al., 1996]

    # Calculate d17O and d18O referenced to VSMOW
    d17O = 1000 * (isol["17R"] / 0.0003799 - 1) # 17R(VSMOW) = 0.0003799 [Li et al., 1988]
    d18O = 1000 * (isol["18R"] / 0.0020052 - 1) # 18R(VSMOW) = 0.0020052 [Baertschi, 1976]

    # Create array of isotope data and return
    deltaVals = np.array([d15N, d17O, d18O]).T
    deltaVals = pd.DataFrame(
        deltaVals, columns=["d15Nbulk","d17O", "d18O"]
    )
...
def calculate_17R(R):
    ...
    saveout["18R"] = 0.0020052 * (saveout["17R"] / 0.0003799) ** (1 / 0.516) # 18R expressed in terms of 17R

    # want the isotope ratios to calculate scrambling
    saveout.to_csv("normalized_ratios.csv")
