...
class Input():
    ...
    def parseratios(self, data):
        ...
        return np.array(data[['size corrected 31R', 
            'size corrected 45R', 'size corrected 46R']].dropna())
    ...
    def parsescrambling(self, data, **Refs):
        ...
        for c in pairings:
            ...
            LHS = LHS[['run_date','ref_tag','size corrected 31R', # only need a subset of columns
            'size corrected 45R', 'size corrected 46R']].dropna().set_index('run_date') # need to drop NaN's

            # pull out data for right-hand side ref. material
            RHS = data[data.ref_tag==c[1]]
            RHS = RHS[['run_date','ref_tag','size corrected 31R',
            'size corrected 45R', 'size corrected 46R']].dropna().set_index('run_date')
